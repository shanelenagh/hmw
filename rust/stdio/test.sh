...
cargo build --release && while IFS= read -r line; do
    echo "$line" | RUST_LOG=debug ./target/release/mcpws -t '[ { "command": "date", "command_parameters": [{ "mcp_param": "dateParams" }], "mcp_tool_spec": { "name": "bigThing", "description": "Awesome method",     "inputSchema": { "properties": { "dateParams": { "title": "Date parameters", "type": "string" } }, "required": [], "type": "object" } } } ]'
done
