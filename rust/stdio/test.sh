#!/bin/bash
cargo build --release --features "debug_log" && while IFS= read -r line; do
    echo "$line" | ./target/release/mcpws -d -t '[ { "command": "date", "command_parameters": [{ "mcp_param": "dateParams" }], "mcp_tool_spec": { "name": "bigThing", "description": "Awesome method",     "inputSchema": { "properties": { "dateParams": { "title": "Date parameters", "type": "string" } }, "required": [], "type": "object" } } } ]'
done
